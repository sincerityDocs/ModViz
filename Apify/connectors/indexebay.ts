...
function normalize(item: Record<string, unknown>): EbayProduct | null {
  ...
  const rawPrice = String(
    item.price ?? item.currentPrice ?? item.itemPrice ?? item.salePrice ?? ''
  );
  ...
  const condition = String(item.condition ?? item.itemCondition ?? '').trim();
...
async function run() {
  ...
  try {
    ...
    console.log(`[ebay-scraper] Valid products ($${MIN_PRICE}-$${MAX_PRICE}, BIN, New): ${valid.length}`);
    ...
    console.log(JSON.stringify({ success: true, connector: CONNECTOR_ID, runId, query: searchQuery, results: valid, metrics: m, budget: budget.summary() }, null, 2));
    ...
    console.error(JSON.stringify({ success: false, connector: CONNECTOR_ID, runId, error: errorMsg, metrics: m, budget: budget.summary() }));
    process.exit(1);
  }
}
